...
public class HttpProtocolTest {
    ...
    @Test
    public void httpProtocolServerTest() throws InterruptedException {
        Map<String, Object> map = new HashMap<>();
        Map<String, Map<String, Object>> localServiceCache = new HashMap();
        localServiceCache.computeIfAbsent(serviceName, k -> new HashMap<>()).computeIfAbsent(version, k -> new HttpProtocolTest());
        map.put("localServiceCache", localServiceCache);
        map.put("baseDir", "C:\\Users\\chlam\\Desktop\\temp");
        map.put("webAppDir", "C:\\Users\\chlam\\Desktop\\temp2");
        ...
        Thread.sleep(1000);
        MyInvocation.MyAppInvocation myAppInvocation = httpProtocolClientTest();
        Assert.assertEquals(myAppInvocation.getName(), "lchhello");
        Assert.assertEquals(myAppInvocation.getAge(), 2);
    }
    
    public MyInvocation.MyAppInvocation httpProtocolClientTest() {
        ...
        Invocation.AppInvocation invocation = Invocation.AppInvocation.newBuilder().setServiceName(serviceName)
                .setVersion(version)
                .setMethodName("test")
